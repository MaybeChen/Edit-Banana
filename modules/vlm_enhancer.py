...
class VLMEnhancer:
    ...
    def recognize_region_elements(self, context: ProcessingContext, regions: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            data = self._parse_json_response_with_debug(
                self.client.analyze_image(self._vlm_image_for_context(context), prompt),
                "vlm_region_elements",
            )
        except Exception as exc:
    ...
    def recognize_structure(self, context: ProcessingContext) -> Dict[str, Any]:
        ...
        try:
            data = self._parse_json_response_with_debug(self.client.analyze_image(self._vlm_image_for_context(context), prompt), "vlm_structure")
        except Exception as exc:
    ...
    def _image_for_stage(context: ProcessingContext, preferred_key: str) -> str:
        ...
        if hasattr(context, "intermediate_results"):
            ...
            if path and os.path.exists(path):
                return path
        return VLMEnhancer._vlm_image_for_context(context)

    @staticmethod
    def _vlm_image_for_context(context: ProcessingContext) -> str:
        """Return the preprocessed full-page image intended for VLM recognition."""
        if hasattr(context, "intermediate_results"):
            path = context.intermediate_results.get("vlm_image_path")
            if path and os.path.exists(path):
                return path
