...
class VLMEnhancer:
    ...
    def _element_summary(elem: ElementInfo) -> Dict[str, Any]:
        return {
            ...
            "start_point": list(elem.arrow_start) if elem.arrow_start is not None else None,
            "end_point": list(elem.arrow_end) if elem.arrow_end is not None else None,
    ...
    @staticmethod
    def _normalize_point(value: Any) -> Optional[tuple]:
        """Normalize a VLM-supplied image-space point to an (x, y) tuple."""
    ...
    def enrich_element_attributes(self, context: ProcessingContext) -> Dict[str, Any]:
        ...
        prompt = (
            ...
            + self._json_only_instruction(
                '{"elements":[{"id":1,"type":"arrow","start_point":[100,120],"end_point":[220,120],"line_style":"dashed","arrow_heads":"end","source_id":2,"target_id":3,"fill_color":"#ffffff","stroke_color":"#333333","stroke_width":2,"corner_radius":8,"confidence":0.9}]}'
            )
            + "只返回需要补充/修正的属性。箭头/连接线优先返回 start_point/end_point 原图像素坐标，最终默认按坐标导出而不是绑定两个元素；arrow_heads 只能是 none/start/end/both；双向箭头必须返回 arrow_heads=both；source_id/target_id 只作为语义关系元数据，如果方向不确定则不要猜；line_style 只能是 solid/dashed/dotted/null；颜色必须是 #RRGGBB。"
        ...
        for item in items:
            ...
            before = {
                ...
                "arrow_start": elem.arrow_start,
                "arrow_end": elem.arrow_end,
                "source_id": elem.source_id,
                "target_id": elem.target_id,
            }
            new_type = self._sanitize_type(item.get("type"))
            ...
            start_point = self._normalize_point(item.get("start_point"))
            end_point = self._normalize_point(item.get("end_point"))
            if start_point is not None:
                elem.arrow_start = start_point
            if end_point is not None:
                elem.arrow_end = end_point
    ...
    def validate_pptx_export(self, context: ProcessingContext, pptx_path: str, round_index: int = 1) -> Dict[str, Any]:
        ...
        prompt = (
            ...
            + self._json_only_instruction(
                '{"score":92,"pass":true,"issues":[{"severity":"medium","description":"missing dashed connector","element_id":3}],"repairs":{"elements":[{"id":3,"start_point":[100,120],"end_point":[220,120],"line_style":"dashed","confidence":0.9}]}}'
            )
