...
class Pipeline:
    ...
    def process_image(self,
        ...
        try:
            ...
            if self._recognition_mode() == "vlm_only":
                ...
                vlm_text_blocks = (
                    structure_result.get("text_blocks")
                    or context.intermediate_results.get("vlm_structure_text_blocks")
                    or []
                )
                if vlm_text_blocks:
                    text_blocks = vlm_text_blocks
                    context.intermediate_results['ocr_text_blocks'] = text_blocks
                    vlm_text_path = os.path.join(img_output_dir, "vlm_structure_text_blocks.json")
                    self._save_json(vlm_text_path, {"text_blocks": text_blocks})
                    context.intermediate_results['vlm_structure_text_blocks_json'] = vlm_text_path
                    print(f"   VLM structure text blocks: {len(text_blocks)}")
    ...
    def _save_vlm_structure_overlay(context: ProcessingContext, output_dir: str) -> str:
        ...
        for block in context.intermediate_results.get("vlm_structure_text_blocks", []):
            geometry = block.get("geometry") if isinstance(block, dict) else None
            if not isinstance(geometry, dict):
                continue
            x = int(geometry.get("x", 0) or 0)
            y = int(geometry.get("y", 0) or 0)
            w = int(geometry.get("width", 0) or 0)
            h = int(geometry.get("height", 0) or 0)
            if w <= 0 or h <= 0:
                continue
            box = [x, y, x + w, y + h]
            draw.rectangle(box, outline="#111827", width=1)
            label = f"text:{block.get('id', '')}"
            draw.text((max(0, x), max(0, y - 12)), label, fill="#111827", font=font)
