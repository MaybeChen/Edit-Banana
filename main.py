    ...
    if not os.path.exists(config_path):
        ...
        return {
            ...
            'multimodal': {
                ...
                'timeout': 60,
                'enabled': False,
                'edge_endpoint_mode': 'coordinates',
                'bind_edges_to_elements': False,
                'use_for': {
...
class Pipeline:
  ...
  <mxGeometry x="{elem.bbox.x1}" y="{elem.bbox.y1}" width="{elem.bbox.width}" height="{elem.bbox.height}" as="geometry"/>
    ...
    def _generate_edge_xml(self, elem, context: ProcessingContext = None) -> str:
        ...
        elem_type = elem.element_type.lower()
        multimodal_cfg = self.config.get("multimodal") or {}
        endpoint_mode = str(multimodal_cfg.get("edge_endpoint_mode", "coordinates")).lower()
        bind_edges = bool(multimodal_cfg.get("bind_edges_to_elements", False))
        relationship_elements = context.elements if context and endpoint_mode in {"relationship", "relationships", "elements"} else None
        start, end = self._infer_edge_points(elem, relationship_elements)
        ...
        elem.arrow_end = end
        source_attr = f' source="{elem.source_id}"' if bind_edges and elem.source_id is not None else ""
        target_attr = f' target="{elem.target_id}"' if bind_edges and elem.target_id is not None else ""
  ...
  <mxGeometry relative="1" as="geometry">
    ...
    def _infer_edge_points(self, elem, elements=None) -> tuple:
        """Infer connector endpoints, preferring explicit VLM coordinates.

        By default edges are exported from sourcePoint/targetPoint coordinates
        instead of binding to source/target cells. Relationship-based endpoints
        are used only when the caller explicitly passes the element list.
        """
        if elem.arrow_start is not None and elem.arrow_end is not None:
            try:
                start = (float(elem.arrow_start[0]), float(elem.arrow_start[1]))
                end = (float(elem.arrow_end[0]), float(elem.arrow_end[1]))
                return start, end
