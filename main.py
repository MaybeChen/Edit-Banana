...
class Pipeline:
    ...
    def process_image(self,
        ...
        preprocess_result = self._preprocess_image_for_pipeline(context, img_output_dir)
        print(
            "   Original: "
            f"{preprocess_result['original_width']}x{preprocess_result['original_height']}; "
            "VLM image: "
            f"{preprocess_result['vlm_width']}x{preprocess_result['vlm_height']} "
            f"(scale={preprocess_result['vlm_scale']:.4f})"
        )
    ...
    @staticmethod
    def _preprocess_image_for_pipeline(context: ProcessingContext, output_dir: str, max_vlm_long_edge: int = 2048) -> Dict[str, Any]:
        """Step 0 image preprocessing.

        Keep the original image as the geometry/crop source, record true pixel
        dimensions, and create a bounded full-page VLM image. VLM coordinates
        remain normalized to 0..1000, so using a thumbnail for full-page
        recognition does not change downstream coordinate conversion.
        """
        from PIL import Image

        os.makedirs(output_dir, exist_ok=True)
        original_source = os.path.abspath(context.image_path)
        suffix = Path(original_source).suffix or ".png"
        original_copy = os.path.join(output_dir, f"original{suffix}")
        if os.path.abspath(original_copy) != original_source:
            shutil.copy2(original_source, original_copy)

        with Image.open(original_source) as img:
            width, height = img.size
            long_edge = max(width, height)
            scale = 1.0
            vlm_path = original_source
            vlm_width, vlm_height = width, height
            if long_edge > max_vlm_long_edge:
                scale = max_vlm_long_edge / float(long_edge)
                vlm_width = max(1, int(round(width * scale)))
                vlm_height = max(1, int(round(height * scale)))
                vlm_path = os.path.join(output_dir, "vlm_thumbnail.png")
                resample = getattr(Image, "Resampling", Image).LANCZOS
                img.convert("RGB").resize((vlm_width, vlm_height), resample).save(vlm_path)

        context.canvas_width = width
        context.canvas_height = height
        metadata = {
            "original_image_path": original_source,
            "saved_original_image_path": original_copy,
            "original_width": width,
            "original_height": height,
            "vlm_image_path": vlm_path,
            "vlm_width": vlm_width,
            "vlm_height": vlm_height,
            "vlm_scale": scale,
            "coordinate_system": "normalized_0_1000",
            "roi_crop_source": "original_image_path",
        }
        context.intermediate_results.update(metadata)
        metadata_path = os.path.join(output_dir, "preprocess_metadata.json")
        Pipeline._save_json(metadata_path, metadata)
        context.intermediate_results["preprocess_metadata_json"] = metadata_path
        return metadata
