...
class _VLMProcessorAdapter:
    ...
    def process(self, context: ProcessingContext) -> ProcessingResult:
        ...
        return ProcessingResult(success=True, elements=context.elements, canvas_width=context.canvas_width, canvas_height=context.canvas_height, metadata=result if isinstance(result, dict) else {'result': result})
def load_config() -> dict:
    config_path = os.path.join(PROJECT_ROOT, 'config', 'config.yaml')
    if not os.path.exists(config_path):
        print(f'Warning: config file not found at {config_path}, using defaults')
        return {'paths': {'input_dir': './input', 'output_dir': './output'}, 'recognition': {'mode': 'sam3_first', 'use_ocr_anchors': True}, 'multimodal': {'mode': 'api', 'api_key': '', 'base_url': '', 'model': '', 'x_hw_id': '', 'x_hw_appkey': '', 'max_tokens': 4000, 'timeout': 60, 'request_text_log_chars': 1200, 'vlm_only_region_max_items': 20, 'vlm_only_ocr_anchor_max_blocks': 30, 'enabled': False, 'use_for': {'prompt_planning': True, 'text_style': True, 'segmentation_refine': True, 'element_refine': False, 'region_refine': False, 'layout_refine': False, 'element_attributes': True, 'export_validate': True}, 'thresholds': {'element_refine_confidence': 0.75, 'region_refine_confidence': 0.7, 'layout_refine_confidence': 0.7, 'vlm_region_confidence': 0.6, 'vlm_connector_confidence': 0.7, 'text_style_confidence': 0.65, 'segmentation_refine_confidence': 0.65, 'element_attribute_confidence': 0.65, 'vlm_structure_confidence': 0.6}}, 'ocr': {'engine': 'paddleocr', 'paddleocr': {'lang': 'ch', 'use_angle_cls': False, 'allow_download': True, 'allow_fallback_to_tesseract': False, 'allow_legacy_fallback': False, 'ocr_version': 'PP-OCRv6', 'text_detection_model_name': 'PP-OCRv6_medium_det', 'text_recognition_model_name': 'PP-OCRv6_medium_rec', 'textline_orientation_model_name': 'PP-LCNet_x1_0_textline_ori', 'text_det_limit_side_len': 64, 'text_det_limit_type': 'min', 'text_det_thresh': 0.3, 'text_det_box_thresh': 0.6, 'text_det_unclip_ratio': 1.5, 'text_rec_score_thresh': 0.0, 'scale': 1.0, 'min_confidence': 0.3}}}
...
class Pipeline:
    ...
    def text_restorer(self):
        if self._text_restorer is None and TextRestorer is not None:
            ...
            self._text_restorer = TextRestorer(formula_engine='none', ocr_engine=ocr_engine, ocr_config=ocr_config)
    ...
    def _process_image_vlm_only(self, context: ProcessingContext, img_output_dir: str, img_stem: str) -> Optional[str]:
        ...
        self._save_json(text_path, {'text_blocks': text_blocks, 'source': 'ocr_anchors' if ocr_anchor_blocks else 'vlm_structure_text_elements'})
    ...
    def _vlm_only_use_ocr_anchors(self) -> bool:
        ...
        multimodal_cfg = self.config.get('multimodal') or {}
        return bool(recognition_cfg.get('use_ocr_anchors', multimodal_cfg.get('vlm_only_use_ocr_anchors', True)))
            ...
            with Image.open(context.image_path) as img:
                ...
                for elem in context.elements:
                    ...
                    if bbox.width <= 1 or bbox.height <= 1:
                        continue
                    crop_box = (max(0, int(bbox.x1)), max(0, int(bbox.y1)), min(rgba.width, int(bbox.x2)), min(rgba.height, int(bbox.y2)))
    ...
    def _extract_vlm_text_blocks(context: ProcessingContext) -> List[Dict[str, Any]]:
        ...
        for elem in context.elements:
            ...
            bbox = elem.bbox
            content = meta.get('content') or meta.get('text') or meta.get('label') or meta.get('value') or ''
            if not str(content).strip():
                continue
            text_blocks.append({'id': len(text_blocks), 'source_element_id': elem.id, 'text': str(content).strip(), 'geometry': {'x': bbox.x1, 'y': bbox.y1, 'width': bbox.width, 'height': bbox.height}, 'font_family': meta.get('font_family') or 'Microsoft YaHei', 'font_size': meta.get('font_size_estimate') or meta.get('font_size') or 12, 'font_weight': meta.get('font_weight') or 'normal', 'font_style': meta.get('font_style') or 'normal', 'font_color': meta.get('font_color') or '#000000', 'text_align': meta.get('text_align') or 'center', 'vertical_align': meta.get('vertical_align') or 'middle', 'confidence': elem.score})
        return text_blocks
    @staticmethod
    ...
    def _save_vlm_structure_overlay(context: ProcessingContext, output_dir: str) -> str:
        ...
        palette = {'container': '#00a6ff', 'rounded rectangle': '#ff5c8a', 'rounded_rectangle': '#ff5c8a', 'rectangle': '#ff5c8a', 'cylinder': '#a855f7', 'arrow': '#2563eb', 'connector': '#2563eb', 'line': '#2563eb', 'icon': '#22c55e', 'picture': '#22c55e', 'logo': '#22c55e', 'chart': '#22c55e'}
        for elem in context.elements:
        ...
        for elem in context.elements:
            ...
            if elem_type in {'arrow', 'line', 'connector'}:
                if self._is_border_like_connector(elem, context):
                    ...
                    if self._recognition_mode() != 'vlm_only' and elem.line_style not in {'dashed', 'dotted'} and self._edge_looks_dashed(elem, context):
    ...
    def _detect_missing_divider_lines(self, context: ProcessingContext, existing_elements: List[ElementInfo]) -> List[ElementInfo]:
        ...
        for x1, y1, x2, y2 in merged:
            if self._overlaps_existing_line([x1, y1, x2, y2], existing_elements):
                continue
            elem = ElementInfo(id=next_id + len(new_elements), element_type='line', bbox=BoundingBox(int(x1), int(y1), int(x2), int(max(y2, y1 + 2))), score=0.8, source_prompt='cv_horizontal_divider', stroke_color='#000000', stroke_width=1, line_style='solid', layer_level=LayerLevel.ARROW.value)
    ...
    def _export_pptx_direct(context: ProcessingContext, elements: List[ElementInfo], text_blocks: List[dict], output_dir: str, img_stem: str) -> Optional[str]:
        from pptx_exporter import export_elements_to_pptx, is_pptx_export_available, missing_pptx_dependency_message
        ...
        return export_elements_to_pptx(elements=elements, text_blocks=text_blocks, canvas_width=context.canvas_width, canvas_height=context.canvas_height, pptx_path=pptx_path)
    ...
    def _find_sharpest_polygon_point(self, points):
        ...
        return best_point
def main():
    parser = argparse.ArgumentParser(description='Edit Banana — image to editable PPTX', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n  python main.py -i input/test.png\n  python main.py\n  python main.py -i test.png --refine\n  python main.py -i test.png --groups image arrow\n        ')
    ...
    for img_path in image_paths:
        result = pipeline.process_image(img_path, output_dir=output_dir, with_refinement=args.refine, with_text=not args.no_text, groups=groups)
        if result:
            success_count += 1
    print(f"\n{'=' * 60}")
