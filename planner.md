...
# 二、统一约定
## 坐标体系
所有 VLM Prompt 都使用实际像素坐标，不再使用归一化坐标。
```text
全图识别：左上角 (0, 0)，右下角为原始图片真实宽高。
ROI 专项识别：左上角 (0, 0)，右下角为当前 ROI 裁剪图真实宽高。
```
全图识别时，坐标相对于整页原始图片。
ROI 专项识别时，坐标相对于当前 ROI 裁剪图；后端负责将 ROI 像素坐标加上 ROI 在原图中的偏移，换算回全图像素坐标。
## 统一元素策略
```text
native_text           可编辑文本框
...
1. 保存原图。
2. 获取真实像素宽高，仅供后端使用。
3. VLM 全图识别直接使用原始图片尺寸语义，输出坐标必须对应原图真实像素。
4. 后续 ROI 必须从原图裁剪，ROI 输出坐标必须对应 ROI 裁剪图真实像素。
5. 后端将 ROI 像素坐标加上 ROI 偏移，换算成全图真实像素坐标。
```
---
  ...
  "regions": [
    {
      "id": "region_001",
      "type": "header",
      "bbox": {"x": 0, "y": 0, "width": 1920, "height": 140},
      "confidence": 0.95
    }
  ],
...
处理：
1. 将 ROI 内实际像素坐标加上 ROI 在原图中的偏移，换算到全图实际像素坐标。
2. 去重。
3. 建立 parent_id。
4. 建立 group_id。
5. 推断 z_index。
